    ...
    <Box>
      {mdSize ? (
        <Card
          ...
          ref={ref}
          className={`${styles.card} ${inView ? styles.isVisible : styles.isHidden}`}>
          <CardContent
            sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <Typography variant="h4" fontWeight="bold">
              {props.index + 1}. {artists}-{track.name}{' '}
            </Typography>
        ...
        <Card
          ...
          <CardContent sx={{ display: 'flex', alignItems: 'center' }}>
            <Box
              sx={
                smSize
                  ? { display: 'flex', alignItems: 'center', marginRight: '64px' }
                  : {
                      display: 'flex',
              ...
              <Typography sx={smSize ? { marginRight: '16px' } : { marginRight: '8px' }}>
                {props.index + 1}. {artists}-{track.name}
              </Typography>
